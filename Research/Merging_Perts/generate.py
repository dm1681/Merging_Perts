...
def write_eqtide(runs_dir_eqonly, n):
...
def write_eqorb(runs_dir_eqorb, n):
	...
	return 1
def write_eqorbmesc(runs_dir_eqorbmesc, n):
	print ('Will be coming!!')
	return 2 

def write_atmesc(runs_dir_atmesc, n):
	runs_dir = runs_dir_atmesc
	# star conditions
	dMass_star = 1
	...
	while i <= n-1:
		...
		dPresXUV_str = str(presxuv[i])
		dAtmGasConst_str = str(atmgasconst[i])
		dThermTemp_str = str(thermtemp[i])
		dFXUV_str = str(fxuv[i])
		dAtmXAbsEffH_str = str(atmxabseffH[i])
		dSatXUVtime_str = str(satxuvtime[i])
		dMass_planet_str = str(mass_b[i] * -1)
		dEnvelopeMass_str = str(env_mass[i] * -1)
		dSemi_str = str(semi_b[i])
		dMass_star_str = str(mass_star[i])
...
if bLoad == False:
	...
	presxuv = np.random.uniform(0.1, 10, n)
	atmgasconst = np.random.uniform(3600, 4157, n)
	thermtemp = np.random.uniform(880, 3000, n)
	fxuv = np.random.uniform(43, 172, n)
	atmmassfrac = np.random.uniform(0.01, 0.1, n)
	atmxabseffH = np.random.uniform(0.1, 0.6, n)
	planetradiusmodel = 'lehmer'
	satxuvtime = np.random.uniform(80e6, 120e6, n)
	env_mass = mass_b * atmmassfrac
	...
	IC['PresXUV'] = presxuv
	IC['AtmGasConst'] = atmgasconst
	IC['ThermTemp'] = thermtemp
	IC['FXUV'] = fxuv
	IC['AtmMassFrac'] = atmmassfrac
	IC['AtmXAbsEffH'] = atmxabseffH
...
if bWrite == 'y':
	...
	presxuv = IC['PresXUV']
	atmgasconst = IC['AtmGasConst']
	thermtemp = IC['ThermTemp']
	fxuv = IC['FXUV']
	atmmassfrac = IC['AtmMassFrac']
	atmxabseffH = IC['AtmXAbsEffH']
	satxuvtime = IC['SatXUVTime']
	env_mass = IC['EnvMass']

	write_which = input('Which options: [eq_only(0), eqorb(1), eqorbmesc(2), atmesc(3)]\n (for help, add \'-h\') ')
	if ((write_which == 'eq_only') or (write_which == '0')):
		print ('Writing input files for %s %s sims'%(str(n),'eqtide'))
		write_eqtide('./runs_eq_only/',n)
	elif ((write_which == 'eqorb') or (write_which == '1')):
		print ('Writing input files for %s %s sims'%(str(n),'eqorb'))
		write_eqorb('./runs_eqorb/',n)
	elif ((write_which == 'eqorbmesc') or (write_which == '2')):
		print ('do eqorbmesc stuff')
		write_eqorbmesc()
	elif ((write_which == 'atmesc')or(write_which == '3')):
		print ('Writing input files for %s %s sims'%(str(n),'atmesc'))
		write_atmesc('./runs_atmesc/',n)
	else:
